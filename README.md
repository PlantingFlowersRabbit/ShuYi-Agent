...
v0.20 增加 AI 章节划分智能体：优先复用本地章节解析脚本，失败后用 DeepSeek + LangChain 生成可复用脚本，并保留本地 Qwen3-TTS 音色生成和台词生成链路。

## 当前状态

- 规格和验收文档已在 `spec/` 与 `docs/` 中建立，当前版本目标见 `spec/v0.20-harness.md`。
...
3. 当前任务相关 `spec/*.md`，v0.20 默认读 `spec/v0.20-harness.md`
...
该脚本检查 harness 文件、docs 索引、manifest 和入库样本音频解码；真实 UI、模型和 TTS 仍需按 `docs/development/v0.20-verification.md` 取证。
