...
书弈 Agent（Shuyi Agent）是基于 Agent 的多人有声书自动配音工作台，面向中文小说配音制作。v0.5.4 取消后端访问令牌流程，支持前端一键后台下载并部署 TTS 模型，同时保留 OpenAI SDK 兼容文本模型配置；运行时不会生成或执行模型返回的 Python 代码。
