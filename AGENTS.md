...
NovelVoice-Agent 是一个小说辅助生成故事配音的 agent 项目。v0.141 交付人工主导的人机协作工作台：导入 txt 小说，先生成轻量章节目录，选择某个章节后才加载并拆分该章正文，维护音色资源库和可新增角色列表，人工确认段落无误后用 Qwen/Qwen3-8B 生成嵌套的可编辑说话人粒度语句草稿，再由用户校正角色、音色、情绪、语速、音量和其他控制文本，最后调用本地 TTS/VoiceDesign 服务生成音频。

本仓库的 harness 目标不是一次性自动化全流程，而是让后续 AI worker 能稳定理解项目边界、测试口径、资源来源、模型合约和人机协作流程，避免未来接入更强自动化或 harness 工程时大规模推倒重来。

## 仓库结构

- `spec/`：当前版本目标、产品范围、架构边界、LLM 语句划分合约和音频合成合约；v0.141 当前目标见 `spec/v0.141-harness.md`。
...
- `frontend/`：React + Vite + TypeScript 前端代码目录，当前提供 v0.141 三页面工作台。
...
- v0.141 仍是人工主导版本，不实现全自动 harness 编排，不自动批量生成整章音频，不自动替用户确认段落或语句划分。
...
- 未来 harness 自动化 provider 预留 DeepSeek：`base_url="https://api.deepseek.com"`，`model="deepseek-v4-flash"`。v0.141 保留配置与文档接口。
