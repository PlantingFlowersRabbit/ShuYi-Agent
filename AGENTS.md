...
NovelVoice-Agent 是一个小说辅助生成故事配音的 agent 项目。v0.24 在 v0.23 TTS 启动就绪门禁基础上调整 AI 语句划分：确认段落后默认以整段落生成语句文本，每个段落内单独触发 LangChain + reflection 的 `AI语句划分`，并保证音频生成使用当前角色绑定的音色参考音频。

本仓库的 harness 目标不是一次性自动化全流程，而是让后续 AI worker 能稳定理解项目边界、测试口径、资源来源、模型合约和人机协作流程，避免未来接入更强自动化或 harness 工程时大规模推倒重来。

## 仓库结构

- `spec/`：当前版本目标、产品范围、架构边界、LLM 语句划分合约和音频合成合约；v0.24 当前目标见 `spec/v0.24-harness.md`。
...
- `frontend/`：React + Vite + TypeScript 前端代码目录，当前提供 v0.24 三页面工作台。
...
- v0.24 仍是人工主导版本，不实现全自动 harness 编排，不自动批量生成整章音频，不自动替用户确认段落。
