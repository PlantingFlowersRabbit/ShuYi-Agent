...
NovelVoice-Agent 是一个小说辅助生成故事配音的 agent 项目。v0.3.0 以 v0.25 AI一键分析 LangGraph 工作流为基线，将语句角色识别升级为批量 LLM 判断，并新增自动角色/音色准备、一键生成配音、一键导出和带约束的角色删除。

本仓库的 harness 目标不是一次性自动化全流程，而是让后续 AI worker 能稳定理解项目边界、测试口径、资源来源、模型合约和人机协作流程，避免未来接入更强自动化或 harness 工程时大规模推倒重来。

## 仓库结构

- `spec/`：当前版本目标、产品范围、架构边界、LLM 语句划分合约和音频合成合约；v0.3.0 为当前增量目标，沿用现有 harness 文档和用户需求。
...
- `frontend/`：React + Vite + TypeScript 前端代码目录，当前提供 v0.3.0 三页面工作台。
...
- v0.3.0 仍保留人工确认边界：AI一键分析可自动准备角色和音色，但必须等待 `角色列表确认完成` 后才继续；配音生成由用户点击 `一键生成配音` 触发，不在分析过程中自动启动。
