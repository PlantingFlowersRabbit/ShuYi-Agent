...
默认访问 `http://127.0.0.1:5173`。本地开发时，`/api` 与 `/outputs` 由 Vite 代理到 `http://127.0.0.1:8000`。需要直连其他后端时，在仓库根目录 `.env` 或前端构建环境设置：

```dotenv
VITE_API_BASE_URL=https://api.example.com/api
```

不要把 API token 或 provider 密钥写进 `VITE_*` 变量；它们会进入浏览器静态文件。
...
1. 可从公网通过 HTTPS 访问；
2. 将 Pages 域名加入 `NOVELVOICE_CORS_ORIGINS`；
3. 为 v1 请求配置 Bearer token；
