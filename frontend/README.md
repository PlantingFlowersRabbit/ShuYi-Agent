...
前端使用 React 19、TypeScript 和 Vite，提供小说导入、角色确认、长句台词拆分、整章播放列表、智能下一步助手、制作阻塞项、项目记忆、配音编排与结果导出工作台。
...
v0.7.2 将主页面入口拆成 **自动配音 / 分步配音 / 纯人工模式**。自动和分步模式都会在 txt/epub 上传后通过“文档解析”调用 `novel_parser` AI 章节划分，失败时先提示人工输入章节正则，仍失败再进入前端本地规则 fallback；两种 Agent 模式都要求人工选择章节。自动配音会自动完成角色分析、音色绑定、台词编排、角色匹配和配音失败恢复，只在导出制作包时等待人工点击；分步配音在角色与音色、台词与角色、全部配音三个节点停给人工审核/修改；纯人工模式则完全由用户手动创建角色、绑定/上传音色、添加/拆分台词、匹配角色和处理失败。

生成前检查、导出前检查和审稿队列合并为 **制作阻塞项** 面板。每条问题展示严重级别、影响、推荐操作和处理状态；批量改角色必须先选择目标角色、预览影响台词数量，再二次确认，避免把一批台词误绑定到默认角色。
...
v0.7.2 延续 **项目记忆**，调用 Story Bible 与 memory-context API 展示角色证据、设定记忆和用户纠错。可信事实、候选事实和已拒绝事实分开展示，用户纠错默认写入 `user_confirmed`，供后续 Agent 优先使用。

## 整章播放与制作包导出

v0.7.2 延续 **试听版 / 交付版 / 后期版** 导出预设，并允许微调片段间停顿、语速、目标峰值、头尾静音裁剪和响度归一化。v0.7.0 在当前章节工具栏补齐整章播放列表控制：一键播放、暂停、继续、上一句、下一句，并沿用台词卡 `playing` 高亮显示当前播放台词。导出制作包现在调用 `/api/v1/projects/{project_id}/exports/{chapter_id}`，下载 URL 同样带 project_id，避免不同项目的导出包串线。
