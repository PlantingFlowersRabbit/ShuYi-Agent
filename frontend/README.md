...
v0.7.1 将主页面收敛为 **一键继续制作**：同一个主按钮根据上下文切换为“开始制作”“继续下一步”或“处理阻塞项”。原制作任务 Planner 不再作为独立三按钮面板暴露，而是变成 **智能下一步助手**，用于解释当前章节状态、生成建议和展示恢复提示。
...
v0.7.1 顶部导航新增 **项目记忆**，调用 Story Bible 与 memory-context API 展示角色证据、设定记忆和用户纠错。可信事实、候选事实和已拒绝事实分开展示，用户纠错默认写入 `user_confirmed`，供后续 Agent 优先使用。

## 整章播放与制作包导出

v0.7.1 在整章播放和导出之上新增 **试听版 / 交付版 / 后期版** 导出预设，并允许微调片段间停顿、语速、目标峰值、头尾静音裁剪和响度归一化。v0.7.0 在当前章节工具栏补齐整章播放列表控制：一键播放、暂停、继续、上一句、下一句，并沿用台词卡 `playing` 高亮显示当前播放台词。导出制作包现在调用 `/api/v1/projects/{project_id}/exports/{chapter_id}`，下载 URL 同样带 project_id，避免不同项目的导出包串线。
