...
install_python_packages() {
  echo "创建或复用 .venv..."
  uv venv .venv --allow-existing --python "$PYTHON_BIN" --link-mode copy "${UV_INDEX_ARGS[@]}"

  echo "安装后端依赖..."
  UV_LINK_MODE=copy uv pip install --python .venv/bin/python --project "$ROOT_DIR" \
  ...
  echo "安装 CPU 版 PyTorch 与 Qwen-TTS..."
  UV_LINK_MODE=copy uv pip install --python .venv/bin/python \
    torch qwen-tts \
    --torch-backend cpu \
    "${UV_INDEX_ARGS[@]}"
...
start_services() {
  ...
  echo "书弈 Agent CPU 模式已启动。按 Ctrl-C 停止。"
